...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream()).filter(item ->itemListOnSale.stream().noneMatch(item.getName()::equals)).map(item -> item.getName()).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(items -> onSale.stream().filter( itemsToBuy -> items.getName().equals(itemsToBuy.getName())).sorted((o1, o2) -> o1.getPrice() - o2.getPrice()).findFirst().map(item -> item.getPrice()).orElse(0)).sum();
